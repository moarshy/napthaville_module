...
from napthaville.persona.prompt_template.print_prompt import print_run_prompts
            ...
            try:
                if "(duration in minutes:" in i:
                    ...
                    if task[-1] == ".":
                        task = task[:-1]
                    if "minutes," in k[1]:
                        duration = int(k[1].split("minutes,")[0].strip())
                    else:
                        duration = int(k[1].split(",")[0].strip())
                    cr += [[task, duration]]

                elif "Duration in minutes:" in i:
                    k = [j.strip() for j in i.split("Duration in minutes:")]
                    task = k[0]
                    if task[-1] == ".":
                        task = task[:-1]
...
def run_gpt_prompt_summarize_conversation(
    ...
    prompt_template = (
        f"{parent_path}/v3_ChatGPT/summarize_conversation_v1.txt"  ########
    )
...
def run_gpt_prompt_decide_to_talk(
    ...
    def create_prompt_input(
        ...
        if (
            len(target_persona_scratch["planned_path"]) == 0
            and "waiting" not in init_act_desc
        ):
    ...
    def create_prompt_input(
        ...
        if len(target_persona_scratch["planned_path"]) == 0:
            ...
            if ":" in target_persona_scratch["act_address"]:
                loc = (
                    target_persona_scratch["act_address"].split(":")[-1]
                    + " in "
                    + target_persona_scratch["act_address"].split(":")[-2]
                )
            target_p_desc = (
                f"{target_persona_name} is already {target_act_desc} at {loc}"
            )
        else:
            loc = ""
            if ":" in target_persona_scratch["act_address"]:
            ...
            target_p_desc = (
                f"{target_persona_name} is on the way to {target_act_desc} at {loc}"
            )
...
def run_gpt_prompt_agent_chat_summarize_ideas(
    ...
    def create_prompt_input(
        persona, target_persona_name, statements, curr_context, test_input=None
    ):
        prompt_input = [
    ...
    prompt_input = create_prompt_input(
        persona, target_persona_name, statements, curr_context
    )  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "Jane Doe is working on a project"  ########
    special_instruction = (
