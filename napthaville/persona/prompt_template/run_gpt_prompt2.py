...
from napthaville.persona.prompt_template.print_prompt import (
    print_run_prompts
)
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False): 
    ...
    gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
                "temperature": 0, "top_p": 1, "stream": False,
                "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = f"{parent_path}/v3_ChatGPT/poignancy_event_v1.txt" ########
...
def run_gpt_prompt_chat_poignancy(persona, event_description, test_input=None, verbose=False): 
    ...
    gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
                "temperature": 0, "top_p": 1, "stream": False,
                "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
    prompt_template = f"{parent_path}/v3_ChatGPT/poignancy_chat_v1.txt" ########
    prompt_input = create_prompt_input(persona, event_description)  ########
    prompt = generate_prompt(prompt_input, prompt_template)
    example_output = "5" ########
    special_instruction = "The output should ONLY contain ONE integer value on the scale of 1 to 10." ########
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
    ...
    prompt_template = f"{parent_path}/v2/wake_up_hour_v1.txt"
...
def run_gpt_prompt_daily_plan(persona, wake_up_hour, test_input=None, verbose=False):
    ...
    prompt_template = f"{parent_path}/v2/daily_planning_v6.txt"
...
def run_gpt_prompt_generate_hourly_schedule(persona, 
    ...
    prompt_template = f"{parent_path}/v2/generate_hourly_schedule_v2.txt"
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  prompt_template = f"{parent_path}/v2/task_decomp_v3.txt"
...
def run_gpt_prompt_action_sector(action_description, 
  ...
  prompt_template = f"{parent_path}/v1/action_location_sector_v1.txt"
...
def run_gpt_prompt_action_arena(action_description, 
  ...
  prompt_template = f"{parent_path}/v1/action_location_object_vMar11.txt"
...
def run_gpt_prompt_action_game_object(action_description, 
  ...
  prompt_template = f"{parent_path}/v1/action_object_v2.txt"
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False): 
  ...
  prompt_template = f"{parent_path}/v3_ChatGPT/generate_pronunciatio_v1.txt" ########
  prompt_input = create_prompt_input(action_description)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = "🛁🧖‍♀️" ########
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False): 
  ...
  prompt_template = f"{parent_path}/v2/generate_event_triple_v1.txt"
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False): 
  ...
  prompt_template = f"{parent_path}/v3_ChatGPT/generate_obj_event_v1.txt" ########
...
def run_gpt_prompt_summarize_conversation(persona, conversation, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v3_ChatGPT/summarize_conversation_v1.txt" ########
...
def run_gpt_prompt_decide_to_talk(
    persona, 
    target_persona_name,
    target_persona_scratch, # persona's scratch memory as dict
    retrieved,
    test_input=None, 
    verbose=False
  ): 

  def create_prompt_input(init_persona, target_persona_name, target_persona_scratch, retrieved, test_input=None): 
    ...
    target_act_desc = target_persona_scratch['act_description']
    if "(" in target_act_desc: 
      target_act_desc = target_act_desc.split("(")[-1][:-1]
    
    if len(target_persona_scratch['planned_path']) == 0 and "waiting" not in init_act_desc: 
      target_p_desc = f"{target_persona_name} is already {target_act_desc}"
    elif "waiting" in init_act_desc:
      target_p_desc = f"{init_persona.name} is {init_act_desc}"
    else: 
      target_p_desc = f"{target_persona_name} is on the way to {target_act_desc}"
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/decide_to_talk_v2.txt"
...
def run_gpt_prompt_decide_to_react(persona, target_persona_name, target_persona_scratch, retrieved, test_input=None, verbose=False): 
  def create_prompt_input(
    ...
    if "(" in target_act_desc: 
      target_act_desc = target_act_desc.split("(")[-1][:-1]
    if len(target_persona_scratch['planned_path']) == 0: 
      loc = ""
      if ":" in target_persona_scratch['act_address']:
        loc = target_persona_scratch['act_address'].split(":")[-1] + " in " + target_persona_scratch['act_address'].split(":")[-2]
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 20, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/decide_to_react_v1.txt"
  prompt_input = create_prompt_input(
    init_persona=persona,
    target_persona_name=target_persona_name,
    target_persona_scratch=target_persona_scratch,
...
def run_gpt_prompt_new_decomp_schedule(persona, 
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 1000, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/new_decomp_schedule_v1.txt"
  prompt_input = create_prompt_input(persona, 
                                     main_act_dur, 
                                     truncated_act_dur, 
                                     start_time_hour,
...
def run_gpt_prompt_agent_chat_summarize_ideas(persona, target_persona_name, statements, curr_context, test_input=None, verbose=False): 
  def create_prompt_input(persona, target_persona_name, statements, curr_context, test_input=None): 
    prompt_input = [persona.scratch.get_str_curr_date_str(), curr_context, persona.scratch.currently, 
                    statements, persona.scratch.name, target_persona_name]
    return prompt_input
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v3_ChatGPT/summarize_chat_ideas_v1.txt" ########
  prompt_input = create_prompt_input(persona, target_persona_name, statements, curr_context)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  example_output = 'Jane Doe is working on a project' ########
  special_instruction = 'The output should be a string that responds to the question.' ########
  fail_safe = get_fail_safe() ########
...
def run_gpt_prompt_agent_chat(
    maze, persona, target_persona_name,
    target_persona_scratch,
    curr_context, init_summ_idea, 
    target_summ_idea, test_input=None, verbose=False): 
  def create_prompt_input(persona, target_persona_name, target_persona_cratch, curr_context, init_summ_idea, target_summ_idea, test_input=None): 
    prev_convo_insert = "\n"
    if persona.a_mem.seq_chat: 
      for i in persona.a_mem.seq_chat: 
        if i.object == target_persona_name: 
    ...
    prompt_input = [persona.scratch.currently, 
                    target_persona_scratch['currently'], 
                    prev_convo_insert,
                    curr_context, 
                    curr_location,
                    persona.scratch.name,
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v3_ChatGPT/agent_chat_v1.txt" ########
  prompt_input = create_prompt_input(
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 250, 
               ...
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/generate_next_convo_line_v1.txt"
...
def run_gpt_prompt_summarize_ideas(persona, statements, question, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v3_ChatGPT/summarize_ideas_v1.txt" ########
...
def run_gpt_prompt_generate_whisper_inner_thought(persona, whisper, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 50, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/whisper_inner_thought_v1.txt"
...
def run_gpt_generate_safety_score(persona, comment, test_input=None, verbose=False): 
  ...
  print ("11")
  prompt_template = f"{parent_path}/safety/anthromorphosization_v1.txt" 
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v3_ChatGPT/generate_focal_pt_v1.txt" ########
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 150, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/generate_focal_pt_v1.txt"
  prompt_input = create_prompt_input(persona, statements, n)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 150, 
               ...
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/insight_and_evidence_v1.txt"
  prompt_input = create_prompt_input(persona, statements, n)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_planning_thought_on_convo(persona, all_utt, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 50, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/planning_thought_on_convo_v1.txt"
...
def run_gpt_prompt_memo_on_convo(persona, all_utt, test_input=None, verbose=False): 
  ...
  gpt_param = {"engine": "text-davinci-002", "max_tokens": 15, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v3_ChatGPT/memo_on_convo_v1.txt" ########
  prompt_input = create_prompt_input(persona, all_utt)  ########
  prompt = generate_prompt(prompt_input, prompt_template)
  ...
  gpt_param = {"engine": "text-davinci-003", "max_tokens": 50, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = f"{parent_path}/v2/memo_on_convo_v1.txt"
  prompt_input = create_prompt_input(persona, all_utt)
  prompt = generate_prompt(prompt_input, prompt_template)
